...
#define size_circularbuffer 1024

#define use_circularbuffer 1
...
uint8_t emptycheck(uint8_t n);
uint8_t fullcheck(uint8_t n);
uint8_t ringlocate(uint8_t i);
uint8_t getelement(uint8_t n,uint8_t current_loc, uint8_t *str);
uint8_t putelement(uint8_t *zz, uint8_t n, uint8_t current_loc,uint8_t *str);
uint8_t circularbuffer[size_circularbuffer];



char warning1[] = {"The buffer is full,ERROR!\0"};
char warning2[] = {"The buffer is empty,ERROR!\0"};
char warning3[] = {"Overfill,ERROR!"};
char warning4[] = {"Overempty,ERROR!"};
char pass[] = {"UNIT TEST PASS!\0"};
char fail[] = {"UNIT TEST FAIL!\0"};

char des[size_circularbuffer];
int st[size_circularbuffer]={0,0,0,0,0,0,0,0};
uint8_t tf[size_circularbuffer];
uint8_t heartbeat_count, h_val = 0x78, t_val=0x79;
float average;
uint8_t *str1;
uint8_t *str2;
int userflag, userflag_temp = 0, tx_count;
...
int Interval,a,b,c,d,h,p,fl,q,kk,count,timer_count;
int t= 0;
...
void powerup(void)
{
	...
	GPIO_PinModeSet(port_TSL_power, 2, gpioModePushPull, 0);				// enable power to MMA
...
void powerdown()
{
    ...
    GPIO_PinModeSet(port_TSL_power, 2, gpioModeDisabled, 0);			// disable power to TSL2561
...
void ADCdmaTransferComplete(unsigned int channel, bool primary, void *user)
{
  ...
  average = convertToCelsius(sum);                                             // Calculate average temperature in celsius
 //str2 = &average;
  //tf[q]=putelement(str2,rb.n,rb.iput,str);
 // q++;
 ...
 //unblockSleepMode(EM2);
  ...
  else if(use_circularbuffer == 0)
  {
	  userflag_temp = 1;                                                     // determine what kind of data is sent in leuart0 : userflag_temp = 1 is temperature data
...
// circular buffer functions
 	 ...
 	 uint8_t ringlocate(uint8_t i)
 	     {
 	         return(i+1)==size_circularbuffer?0:i+1;
 	     }
 	 ...
 	 uint8_t getelement(uint8_t n,uint8_t current_loc, uint8_t *str)
 	     {
 	         ...
 	         {   int ii=0;
 	             while (warning2[ii] != '\0'){
 	             printf("%c",warning2[ii]);
 	             ii++;
 	             }
 	             return 1;
 	         }
 	         else if(st[current_loc]==1)
 	         {
 	             rb.iget=ringlocate(current_loc);                                                          //update rb.iget
 	             rb.n--;
 	 ...
 	 uint8_t putelement(uint8_t *zz, uint8_t n, uint8_t current_loc,uint8_t *str)
 	     {
 	         ...
 	         return 1;
 	             ...
 	             *(str+current_loc)=*zz;
 	             ...
 	             rb.iput=ringlocate(current_loc);
 	             rb.n++;
 	             st[current_loc]=1;
 	             return 0;
  ...
  void LEUART0_IRQHandler(void)
  {
	...
	if(use_circularbuffer == 1)
	{
        ...
        tf[kk]=getelement(rb.n,rb.iget,str);

  	    LEUART0->TXDATA = des[t];

  	    while (!(LEUART0->IF & LEUART_IF_TXC));

  	    if ( t<size_circularbuffer )
  	    {
  	    	t++;
  	    }
  	    else t=0;
	...
	else if(use_circularbuffer == 0)
	{
		...
		else if(userflag_temp == 0)
		{
			LEUART0->TXDATA = h_val;
			while (!(LEUART0->IF & LEUART_IF_TXC));

			LEUART0->TXDATA = heartbeat_count;
			while (!(LEUART0->IF & LEUART_IF_TXC));
...
void LETIMER0_IRQHandler(void){
	...
	else if ((currentFlags & LETIMER_IF_UF) != 0)
	{
        ...
        {
  		str1 = &h_val;
  		tf[q]=putelement(str1,rb.n,rb.iput,str);
  		q++;
...
/* Routine to find the average temperation in celsius and change LED state accordingly */
		...
		else if( Interval == 1 )
		{
	    	...
	    	if(i2c_txBuffer5 >= 220 && i2c_txBuffer5 < 245)
	    	{
 ...
 int main(void){
	...
	CHIP_Init(); // initialize all errata
	   ...
	   str=(char*)malloc(size_circularbuffer);
	   str = circularbuffer;
